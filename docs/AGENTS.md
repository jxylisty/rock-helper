...
公式口径已与开源社区验证项目 roco-cal（github.com/180sans/roco-cal）逐项对拍：
克制表 18×18=324 格完全一致；面板公式代数等价；以下为确认后的口径。
...
- 性格提升项倍率：1 + (10 + 2*星级)%（5星 = 1.2）
- 性格降低项倍率：0.9
- 属性一致加成：1.25
- 克制倍率模型（data/config/typeChart.js getAttrMultiplier）：每对攻/防独立查格，
  n 个克制相乘为 (1+n)（单克 2、双克 3），其余直接连乘——
  双重抵抗 = 0.5×0.5 = **0.25**（不是 1/3），克制+抵抗 = 1，1克+2抗 = 0.5
- 面板：基础面板**四舍五入** → 乘性格 → 四舍五入 → 加星级裸加成（HP +star*20，其余 +star*10）
- 伤害常量：(等级*0.45 + 10) / 41（60级 ≈ 0.90244，随等级变化，**不是固定 0.9**）
- 伤害 = 攻/防 × 常量 × 有效威力（含本系1.25与克制倍率）× 增伤/减伤修饰 × 连击，最终下限 1

## 愿力冲击规则

- 愿力基础威力 80，成功应对对方状态技能后按 200（80×2.5）结算
- 物攻/魔攻由精灵自身攻击/魔攻面板较高项决定
- 属性随血脉自选（18 系）；愿力属性 ≠ 自身属性时无本系 1.25
- 每场 2 次、能耗 2、顶替一号技能位
- 推荐实现在 utils/wishPowerAdvisor.js（纯克制规则计算，零人工维护数据），
  整合在精灵详情页"愿力冲击推荐"折叠卡与属性值计算页"愿力冲击"快捷块
